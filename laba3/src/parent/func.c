...
void out_all(){
  printf ("Parent stats: pid = %d, ppid = %d, num childs = %d\n", getpid(), getppid(), (int)amount);
  printf ("Childs stats:\n");
  
  for (size_t i=0;i<amount;i++){
    printf ("C_%d: pid = %d, ppid = %d, allowed = %d\n", (int)i, child_array[i].pid, getpid(),child_array[i].print_allowed);
  }
