...
void atexit_handler(void) {
        // Если текущий процесс является родительским процессом
    ...
    } else if (getppid() == parent_pid) { // Если текущий процесс является дочерним процессом
...
// Удаляем разделяемую память
    if (shm_unlink(SHM_OBJECT)) {
     ...
     // Удаляем семафоры MUTEX, FREE_SPACE и ITEMS
    if (sem_unlink(MUTEX) ||
